...
{
    public class SaleSaleReturnUseController : AppController
    {
        ...
        public SaleSaleReturnUseController()
        {
        ...
        // SAVE ALL INFORMATION from cart(Sale/SaleReturn/Use Master) TO Asl_lOG Database Table.
        ...
        public void Update_PharmacyMaster_LogData(PharmacyDTO model)
        {
            ...
            aslLog.LOGDATA = Convert.ToString("Sale/Sale-Return/Use(Update Menu)-Payment Information. Trasaction type: " + transType + ",\nDate: " + model.TRANSDT + ",\nMemo No: " + model.TRANSNO + ",\nDepartment: " + model.DepartmentName + ",\nPatient Type: " + model.PATIENTTP + ",\nPatient ID: " + model.PATIENTID + ",\nPatient Name: " + model.PatientName + ",\nTotal Amount: " + model.TOTAMT + ",\nVate Rate: " + model.VATRT + ",\nVate Amount: " + model.VATAMT + ",\nDiscount: " + model.DISCOUNT + ",\nAmount: " + model.NETAMT + ",\nRemarks: " + model.REMARKS + ".");
            aslLog.USERPC = strHostName;
        ...
        // SAVE ALL INFORMATION from grid(Sale/SaleReturn/Use data) TO Asl_lOG Database Table.
        public void Insert_Pharmacy_LogData(PharmacyDTO model)
        {
            ...
            aslLog.LOGDATA = Convert.ToString("Sale/Sale-Return/Use. Trasaction type: " + transType + ",\nDate: " + model.TRANSDT + ",\nMemo No: " + model.TRANSNO + ",\nDepartment: " + model.DepartmentName + ",\nPatient Type: " + model.PATIENTTP + ",\nPatient ID: " + model.PATIENTID + ",\nPatient Name: " + model.PatientName + ",\nItem name: " + model.MEDINM + ",\nQty:" + model.QTY + ",\nRate:" + model.RATE + ",\nAmount:" + model.AMOUNT + ".");
            aslLog.USERPC = strHostName;
        ...
        // Edit ALL INFORMATION from Grid(Sale/SaleReturn/Use data) TO Asl_lOG Database Table.
        ...
        // Delete ALL INFORMATION from Grid(Sale/SaleReturn/Use data) TO Asl_lOG Database Table.
        ...
        // Delete ALL INFORMATION from Grid(Sale/SaleReturn/Use Data) TO ASL_DELETE Database Table.
        public void Delete_Pharmacys_LogDelete(PharmacyDTO model)
        {
            ...
            AslDelete.DELDATA = Convert.ToString("Sale/Sale-Return/Use. Trasaction type: " + transType + ",\nDate: " + model.TRANSDT + ",\nMemo No: " + model.TRANSNO + ",\nDepartment: " + model.DepartmentName + ",\nPatient Type: " + model.PATIENTTP + ",\nPatient ID: " + model.PATIENTID + ",\nPatient Name: " + model.PatientName + ",\nItem name: " + model.MEDINM + ",\nQty:" + model.QTY + ",\nRate:" + model.RATE + ",\nAmount:" + model.AMOUNT + ".");
        ...
        // Delete ALL INFORMATION from cart(Sale/SaleReturn/Use Master) TO ASL_DELETE Database Table.
        public void Delete_PharmacyMaster_LogDelete(PharmacyDTO model)
        {
            ...
            AslDelete.DELDATA = Convert.ToString("Sale/Sale-Return/Use-Payment Information. Trasaction type: " + transType + ",\nDate: " + model.TRANSDT + ",\nMemo No: " + model.TRANSNO + ",\nDepartment: " + model.DepartmentName + ",\nPatient Type: " + model.PATIENTTP + ",\nPatient ID: " + model.PATIENTID + ",\nPatient Name: " + model.PatientName + ",\nTotal Amount: " + model.TOTAMT + ",\nVate Rate: " + model.VATRT + ",\nVate Amount: " + model.VATAMT + ",\nDiscount: " + model.DISCOUNT + ",\nAmount: " + model.NETAMT + ",\nRemarks: " + model.REMARKS + ".");
            AslDelete.USERPC = strHostName;
        ...
        public ActionResult CreatePost(PharmacyDTO model, string command)
        {
            if (command == "Save" || command == "A4" || command == "POS")
            {
                ...
                {
                    TempData["Sale/SaleReturn/Use-Create"] = "Please input valid data !";
                ...
                if (checkPharmacyMaster.Count == 0)
                {
                    ...
                    if (model.TRANSTP == "SALE")
                    {
                        ...
                        pharmacyMaster.BILLID = Convert.ToInt64(model.COMPID + "109");
                    }
                    else if (model.TRANSTP == "IRTS")
                    {
                    ...
                    {
                        pharmacyMaster.DEPTFR = model.DEPT_FR_TO;
                        pharmacyMaster.BILLID = Convert.ToInt64(model.COMPID + "110");
                    }
                    ...
                    TempData["Sale/SaleReturn/Use-Create"] = "Create successfully.";
        ...
        public ActionResult UpdatePost(PharmacyDTO model, string command)
        {
            if (command == "Save" || command == "A4" || command == "POS")
            {
                ...
                TempData["Sale/SaleReturn/Use-Update"] = "Update successfully.";
        ...
        public ActionResult DeletePost(PharmacyDTO model, string command)
        {
            if (command == "Delete")
            {
                ...
                TempData["Sale/SaleReturn/Use-Delete"] = "Delete successfully.";
        ...
        public JsonResult GetMemoNO_DepartmentInfo(string type, string compid, string date)
        {
            ...
            Int64 deptid = 0;
            if (type == "SALE" || type == "IRTS")
            {
                deptid = Convert.ToInt64(comapnyid + "201");
            }
            else if (type == "IUSE")
            {
                deptid = Convert.ToInt64(comapnyid + "202");
            }

            var findDepartmentName = from m in db.HMS_DEPT where m.COMPID == comapnyid && m.DEPTID == deptid && m.DEPTTP == "PHARMACY" select new { m.DEPTID, m.DEPTNM };
            foreach (var y in findDepartmentName)
            {
                departmentID = Convert.ToString(y.DEPTID);
                departmentName = y.DEPTNM;
            }


            var result = new { TRANSNO = memoNo, DEPTID = departmentID, DEPTNM = departmentName };
        ...
        public JsonResult TagSearch_patient(string term, string compid, string patientType)
        {
            ...
            {
                var tags = from p in db.HMS_IpdDbSet where p.COMPID == companyid select new { p.PATIENTYYID };
                foreach (var tag in tags)
                {
            ...
            {
                var tags = from p in db.HMS_OPDMST where p.COMPID == companyid select new { p.PATIENTYYID };
                foreach (var tag in tags)
                {
                    result.Add(tag.PATIENTYYID.ToString());
                }
        ...
        public JsonResult keyword_GetPatient(string changedText, string companyid, string patientType)
        {
            ...
            if (patientType == "INDOOR")
            {
                var tags = from p in db.HMS_IpdDbSet where p.COMPID == compid select new { p.PATIENTYYID };
                foreach (var tag in tags)
                {
                    patientList.Add(tag.PATIENTYYID.ToString());
                }
            }
            else if (patientType == "OUTDOOR")
            {
                var tags = from p in db.HMS_OPDMST where p.COMPID == compid select new { p.PATIENTYYID };
                foreach (var tag in tags)
                {
                    patientList.Add(tag.PATIENTYYID.ToString());
                }
            ...
            if (regex.IsMatch(itemId))
            {
                ...
                if (patientType == "INDOOR")
                {
                    var findpatientInfo = db.HMS_IpdDbSet.Where(n => n.PATIENTYYID == patientYearID &&
                                                              n.COMPID == compid).Select(n => new
                                                              {
                                                                  n.PATIENTNM,
                                                                  n.PATIENTYY,
                ...
                {
                    var findpatientInfo = db.HMS_OPDMST.Where(n => n.PATIENTYYID == patientYearID &&
                                                               n.COMPID == compid).Select(n => new
                                                               {
                                                                   n.PATIENTNM,
                                                                   n.PATIENTYYID,
        ...
        public JsonResult keyword_GetTransNO(string changedText, string companyid, string transactionType, string transactionDate)
        {
            ...
            String departmentName = "", Patientid = "", patientName = "", patientType = "", transDate = "";
            Int64 department_For_To_id = 0, PatientYearID = 0, patientYear = 0;
            ...
            if (regex.IsMatch(itemId))
            {
                ...
                foreach (var n in findInfo)
                {
                    ...
                    else if (transactionType == "IRTS")
                    {
                        department_For_To_id = Convert.ToInt64(n.DEPTTO);
                    }
                    else if (transactionType == "IUSE")
                    {
                        department_For_To_id = Convert.ToInt64(n.DEPTFR);
                    }
                ...
                if (patientType == "INDOOR")
                {
                    var findpatientInfo = db.HMS_IpdDbSet.Where(m => m.PATIENTID == Patientid && m.PATIENTYY == patientYear && m.PATIENTYYID == PatientYearID &&
                                                              m.COMPID == compid).Select(m => new
                                                              {
                                                                  m.PATIENTNM,

                                                              });
                    foreach (var a in findpatientInfo)
                    {
                        patientName = a.PATIENTNM;
                    }
                }
                else if (patientType == "OUTDOOR")
                {
                    var findpatientInfo = db.HMS_OPDMST.Where(n => n.PATIENTID == Patientid && n.PATIENTYYID == PatientYearID &&
                                                              n.COMPID == compid).Select(n => new
                                                              {
                                                                  n.PATIENTNM,
            ...
            var result = new { TRANSNO = itemId, TRANSDT = transDate, DEPT_FR_TO = department_For_To_id, PATIENTYYID = PatientYearID, PATIENTYY = patientYear, PATIENTTP = patientType, DepartmentName = departmentName, PATIENTID = Patientid, PatientName = patientName, };
            return Json(result, JsonRequestBehavior.AllowGet);
