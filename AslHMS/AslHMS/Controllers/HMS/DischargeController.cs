...
{
    public class DischargeController : AppController
    {
        ...
        // SAVE ALL INFORMATION from HMS_CDISCHG TO Asl_lOG Database Table.
        public void Insert_Discharge_LogData(DischargeDTO model)
        {
            ...
            aslLog.TABLEID = "HMS_CDISCHG";

            DateTime date1 = Convert.ToDateTime(model.DISCHGDT);
            string ddt = date1.ToString("dd-MMM-yyyy");
            var dischargeDate = ddt;

            aslLog.LOGDATA = Convert.ToString("Discharge Certificate Entry Page. Transaction No: " + model.TRANSNO + ",\nPatient Type: " + model.PATIENTTP + ",\nPatient ID: " + model.PATIENTID + ",\nPatient Name: " + model.PatientName + ",\nDischarge Date: " + dischargeDate + ",\nDischarge Time: " + model.DISCHGTM + ",\nDiagnosis: " + model.DIAGNOSIS + ",\nOP Note: " + model.OPNOTE + ",\nTreatment: " + model.TREATMENT + ",\nAdvice: " + model.ADVICE + ",\nRemarks: " + model.REMARKS + ".");
            aslLog.USERPC = model.USERPC;
            db.AslLogDbSet.Add(aslLog);
        }

        // Edit ALL INFORMATION from HMS_CDISCHG TO Asl_lOG Database Table.
        public void Update_Discharge_LogData(DischargeDTO model)
        {
            TimeZoneInfo timeZoneInfo;
            timeZoneInfo = TimeZoneInfo.FindSystemTimeZoneById("Central Asia Standard Time");
            DateTime PrintDate = TimeZoneInfo.ConvertTime(DateTime.Now, timeZoneInfo);
        ...
        // Delete ALL INFORMATION from HMS_CDISCHG TO Asl_lOG Database Table.
        public void Delete_Discharge_DELETE(DischargeDTO model)
        {
            TimeZoneInfo timeZoneInfo;
            timeZoneInfo = TimeZoneInfo.FindSystemTimeZoneById("Central Asia Standard Time");
            DateTime PrintDate = TimeZoneInfo.ConvertTime(DateTime.Now, timeZoneInfo);
        ...
        // Delete ALL INFORMATION from HMS_CDISCHG TO ASL_DELETE Database Table.
        public void Delete_Discharge_LogData(DischargeDTO model)
        {
            TimeZoneInfo timeZoneInfo;
            timeZoneInfo = TimeZoneInfo.FindSystemTimeZoneById("Central Asia Standard Time");
            DateTime PrintDate = TimeZoneInfo.ConvertTime(DateTime.Now, timeZoneInfo);
            ...
            AslDelete.TABLEID = "HMS_CDISCHG";

            DateTime date1 = Convert.ToDateTime(model.DISCHGDT);
            string ddt = date1.ToString("dd-MMM-yyyy");
            var dischargeDate = ddt;

            AslDelete.DELDATA = Convert.ToString("Discharge Certificate Entry Page. Transaction No: " + model.TRANSNO + ",\nPatient Type: " + model.PATIENTTP + ",\nPatient ID: " + model.PATIENTID + ",\nPatient Name: " + model.PatientName + ",\nDischarge Date: " + dischargeDate + ",\nDischarge Time: " + model.DISCHGTM + ",\nDiagnosis: " + model.DIAGNOSIS + ",\nOP Note: " + model.OPNOTE + ",\nTreatment: " + model.TREATMENT + ",\nAdvice: " + model.ADVICE + ",\nRemarks: " + model.REMARKS + ".");
        ...
        public ActionResult Create(DischargeDTO model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (findPreviousData.Count == 0)
                {
                    ...
                    discharge.COMPID = model.COMPID;
                    discharge.TRANSDT = Convert.ToDateTime(model.TRANSDT);
                    discharge.TRANSYY = model.TRANSYY;
                    discharge.TRANSNO = model.TRANSNO;
                    discharge.PATIENTTP = model.PATIENTTP;
                    discharge.PATIENTYY = model.PATIENTYY;
                    discharge.PATIENTYYID = model.PATIENTYYID;
                    discharge.PATIENTID = model.PATIENTID;
                    discharge.DISCHGDT = Convert.ToDateTime(model.DISCHGDT);
                    discharge.DISCHGTM = model.DISCHGTM;
                    discharge.DIAGNOSIS = model.DIAGNOSIS;
                    discharge.OPNOTE = model.OPNOTE;
                    discharge.TREATMENT = model.TREATMENT;
                    discharge.ADVICE = model.ADVICE;
                    discharge.REMARKS = model.REMARKS;

                    discharge.USERPC = model.USERPC;
                    discharge.INSIPNO = model.INSIPNO;
                    discharge.INSTIME = model.INSTIME;
                    discharge.INSUSERID = Convert.ToInt64(model.INSUSERID);
                    discharge.INSLTUDE = model.INSLTUDE;

                    Insert_Discharge_LogData(model);
                    db.HmsDischargeDbSet.Add(discharge);
                    db.SaveChanges();
                    TempData["DischargeCreationMessage"] = "Discharge Certificate successfully registered! ";
                }
                else
                {
                    TempData["DischargeCreationMessage"] = "Patient ID:" + model.PATIENTYYID + " already enrolled!!!";
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Update(DischargeDTO model)
        {
            if (ModelState.IsValid)
            {
                var result_Find = (from n in db.HmsDischargeDbSet
                ...
                if (result_Find == 1)
                {
                    ...
                    Update_Discharge_LogData(model);
                    db.SaveChanges();

                    Discharge discharge = new Discharge();
                    discharge.ID = model.ID;
                    discharge.COMPID = model.COMPID;
                    discharge.TRANSDT = Convert.ToDateTime(model.TRANSDT);
                    discharge.TRANSYY = model.TRANSYY;
                    discharge.TRANSNO = model.TRANSNO;
                    ...
                    discharge.INSIPNO = model.INSIPNO;
                    discharge.INSTIME = model.INSTIME;
                    discharge.INSUSERID = model.INSUSERID;
                    discharge.INSLTUDE = model.INSLTUDE;
                    discharge.UPDIPNO = model.UPDIPNO;
                    discharge.UPDTIME = model.UPDTIME;
                    discharge.UPDUSERID = model.UPDUSERID;
                    discharge.UPDLTUDE = model.UPDLTUDE;

                    db.Entry(discharge).State = EntityState.Modified;
                    db.SaveChanges();

                    TempData["DischargeUpdateMessage"] = "Discharge certificate successfully updated!";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(DischargeDTO model)
        {
            if (ModelState.IsValid)
            {
                var result_Find = (from n in db.HmsDischargeDbSet
                ...
                if (result_Find == 1)
                {
                    Discharge deleteModel = new Discharge();
                    deleteModel.ID = model.ID;
                    deleteModel.COMPID = model.COMPID;
                    deleteModel.TRANSYY = model.TRANSYY;
                    deleteModel.TRANSNO = model.TRANSNO;
                    deleteModel = db.HmsDischargeDbSet.Find(deleteModel.ID, deleteModel.COMPID, deleteModel.TRANSYY, deleteModel.TRANSNO);
                    ...
                    Delete_Discharge_DELETE(model);
                    Delete_Discharge_LogData(model);

                    db.HmsDischargeDbSet.Remove(deleteModel);
                    db.SaveChanges();
                    TempData["DischargeDeleteMessage"] = "Discharge certificate successfully deleted!";
        ...
        public JsonResult keyword_PatientInfo(string changedText, string patientYear)
        {
            ...
            if (regex.IsMatch(itemId))
            {
                ...
                var checkDischargeTable = (from m in db.HmsDischargeDbSet
                                           where m.COMPID == compid && m.PATIENTYY == pYear && m.PATIENTYYID == patientYearID
                                           select m).ToList();
                if (checkDischargeTable.Count == 1)
                {
        ...
        public JsonResult TagSearch_UD_Discharge(string term, Int64 compid, Int64 patientYear)
        {
            List<string> result = new List<string>();
            //var compid = Convert.ToInt16(System.Web.HttpContext.Current.Session["loggedCompID"].ToString());
            var tags = from p in db.HmsDischargeDbSet where p.COMPID == compid && p.PATIENTYY == patientYear select new { p.PATIENTYYID };
            foreach (var tag in tags)
            {
                result.Add(tag.PATIENTYYID.ToString());
            }
        ...
        //AutoComplete Birth (Update & Delete page)
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult keyword_UD_Discharge(string changedText, string compid, string pYear)
        {
            //var compid = Convert.ToInt16(System.Web.HttpContext.Current.Session["loggedCompID"].ToString());
            String itemId = "";
            ...
            var tags = from p in db.HmsDischargeDbSet
            ...
            String transDate = "", dischargeDate = "", dischargeTime = "", patientID = "", patientNm = "", diagnosis = "", opnote = "", treatment = "", advice = "", remarks = "", inserttime = "", insertIpno = "", insltude = "";
            ...
            {
                Int64 patientYearID = Convert.ToInt64(itemId);
                var findInfo = db.HmsDischargeDbSet.Where(n => n.PATIENTYYID == patientYearID && n.PATIENTYY == patientYear &&
                ...
                foreach (var n in findInfo)
                {
                    ...
                    string dDate = Convert.ToString(n.DISCHGDT);
                    DateTime dischargedate = DateTime.Parse(dDate);
                    dischargeDate = dischargedate.ToString("dd-MMM-yyyy");

                    dischargeTime = n.DISCHGTM;
            ...
            var result = new
            {
                ...
                PATIENTID = patientID,
                PATIENTNM = patientNm,
                DISCHGDT = dischargeDate,
                DISCHGTM = dischargeTime,
                DIAGNOSIS = diagnosis,
                OPNOTE = opnote,
                TREATMENT = treatment,
                ADVICE = advice,
