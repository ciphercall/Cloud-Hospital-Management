...
namespace AslHMS.Controllers.Api
{
    public class ApiTestNVController : ApiController
    {
        ...
        [Route("apiTestNV/ResultGroupList")]
        [HttpGet]
        public IEnumerable<TestNVDTO> GetRestGroupList(string query, string query2, string query3)
        {
            using (var db = new AslHMSApiContext())
            {
                Int64 compid = Convert.ToInt64(query2);
                Int64 testid = Convert.ToInt64(query3);
                return String.IsNullOrEmpty(query) ? db.HMS_TESTNV.AsEnumerable().Select(b => new TestNVDTO { }).ToList() :
                db.HMS_TESTNV.Where(p => p.RESTGR.StartsWith(query) && p.COMPID == compid && p.TESTID == testid).Distinct().Select(
                          x =>
                          new
                          {
                              ResultGroup = x.RESTGR,
                            

                          })
                .AsEnumerable().Distinct().Select(a => new TestNVDTO
                {
                    RestGroupName = a.ResultGroup,
        ...
        [System.Web.Http.Route("apiTestNV/DynamicResultGroup")]
        [System.Web.Http.HttpGet]
        public IEnumerable<TestNVDTO> ResultGroupautocomplete(string changedText, string changedText2, string changedText3)
        {
            using (var db = new AslHMSApiContext())
            {
                Int64 compid = Convert.ToInt64(changedText2);
                ...
                var rt = db.HMS_TESTNV.Where(n => n.RESTGR.StartsWith(changedText) && n.COMPID == compid && n.TESTID == testid).Distinct().Select(n => new
                {
                    headname = n.RESTGR
                ...
                var findid = (from n in db.HMS_TESTNV where n.RESTGR == name && n.COMPID == compid && n.TESTID == testid select n).Distinct().ToList();
                if (findid.Count != 0)
                {
                    return db.HMS_TESTNV.Where(p => p.RESTGR == name && p.COMPID == compid && p.TESTID == testid).Distinct().Select(
                        x =>
                            new
                            {
                                ResultGroup = x.RESTGR
                ...
                else
                {
                    return db.HMS_TESTNV.AsEnumerable().Select(a => new TestNVDTO
                    {
                        RestGroupName = Convert.ToString(name),
