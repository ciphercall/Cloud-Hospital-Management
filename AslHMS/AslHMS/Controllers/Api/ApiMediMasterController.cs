...
namespace AslHMS.Controllers.Api
{
    public class ApiMediMasterController : ApiController
    {
        ...
        public IEnumerable<MediMstDTO> GetCategoryData(string Compid)
        {
            ...
            var find_GridData = (from t1 in db.HmsMediMstDbSet
        ...
        public HttpResponseMessage AddData(MediMstDTO model)
        {
            ...
            var check_data = (from n in db.HmsMediMstDbSet where n.COMPID == model.COMPID && n.MCATNM == model.MCATNM select n).ToList();
            if (check_data.Count == 0)
            {
                var find_data = (from n in db.HmsMediMstDbSet where n.COMPID == model.COMPID select n.MCATID).ToList();
                ...
                {
                    Int64 find_Max_MCATID = Convert.ToInt64((from n in db.HmsMediMstDbSet where n.COMPID == model.COMPID select n.MCATID).Max());
                ...
                db.HmsMediMstDbSet.Add(mediMst);
                db.SaveChanges();
        ...
        public HttpResponseMessage UpdateData(MediMstDTO model)
        {
            ...
            if (check_data.Count == 0)
            {
                var data_find = (from n in db.HmsMediMstDbSet where n.ID == model.ID && n.COMPID == model.COMPID && n.MCATID == model.MCATID select n).ToList();
        ...
        public HttpResponseMessage DeleteData(MediMstDTO model)
        {
            ...
            var findChildData = (from n in db.HmsMediCareDbSet
                                 where
            ...
            {
                deleteModel = db.HmsMediMstDbSet.Find(deleteModel.ID, deleteModel.COMPID, deleteModel.MCATID);
                db.HmsMediMstDbSet.Remove(deleteModel);
                db.SaveChanges();
