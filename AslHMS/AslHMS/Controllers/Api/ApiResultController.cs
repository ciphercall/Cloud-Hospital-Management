...
namespace AslHMS.Controllers.Api
{
    public class ApiResultController : ApiController
    {
        ...
        [System.Web.Http.Route("api/Result/PatientYId")]
        [System.Web.Http.HttpGet]
        public IEnumerable<ResultDTO> GetPatientId(string query, string query2,string query3)
        {
            using (var db = new AslHMSApiContext())
            {
                ...
                if (query3 == "INDOOR")
                {
                    var tags = from p in db.HMS_IpdDbSet
                               ...
                               select new { p.PATIENTYYID,p.PATIENTNM,p.TRANSDT,p.MOBNO};
        ...
        [System.Web.Http.HttpGet]
        public IEnumerable<ResultDTO> autocompleteForResult(string changedText, string changedText2,string changedText3)
        {
            using (var db = new AslHMSApiContext())
            {
                ...
                if (changedText3 == "INDOOR")
                {
                    tags = from p in db.HMS_IpdDbSet
                        where p.COMPID == compid
                        select new {p.PATIENTYYID};
                    string date = td.ToString("yyyy-MM-dd");
                    ...
                    if (findid.Count != 0)
                    {
                        ...
                        foreach (var item in returndata)
                        {
                            ...
                            yield return new ResultDTO
                            {
                                ...
                                PatientYearID = Convert.ToInt64(item.patientid),
                ...
                {
                    Int64 yearid = Convert.ToInt64(name);
                    var findid = (from n in db.HMS_IpdDbSet where n.PATIENTYYID == yearid && n.COMPID == compid select n).ToList();
                    ...
                    if (findid.Count != 0)
                    {
                        ...
                        var returndata = (from t1 in db.HMS_IpdDbSet
                                          //join t2 in db.HMS_OPD on t1.COMPID equals t2.COMPID
                                          where t1.COMPID == compid && t1.PATIENTYYID == yearid
                                          select new
                                          {
        ...
        public IEnumerable<ResultDTO> Get(string Compid, string Transdt, string Patientid, string Testid, string Insuserid, string Insltude)
        {
            ...
            if (ache_kina_data.Count == 0)
            {
                ...
                var patientyid =
                    (from n in db.HMS_OPDMST where n.COMPID == compid && n.PATIENTID == Patientid select n).ToList();
                Int64 PatientyearID = 0;
                foreach (var opdMaster in patientyid)
                {
                    PatientyearID = Convert.ToInt64(opdMaster.PATIENTYYID);
                }
        ...
        public HttpResponseMessage SaveData(ResultDTO model)
        {
            ...
            if (duplicate_data_test.Count > 0)
            {
                ...
                if (id == model.ID)
                {
                    ...
                    var patientyid =
                  (from n in db.HMS_OPDMST where n.COMPID == model.COMPID && n.PATIENTID == model.PatientId select n).ToList();
                    Int64 PatientyearID = 0;
                    foreach (var opdMaster in patientyid)
                    {
                        PatientyearID = Convert.ToInt64(opdMaster.PATIENTYYID);
                    ...
                    foreach (var item in data_find)
                    {
                        ...
                        item.PATIENTYYID = PatientyearID;
