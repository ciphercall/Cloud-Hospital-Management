...
namespace AslHMS.Controllers.Api
{
    public class ApiTestMasterController : ApiController
    {
        ...
        [System.Web.Http.HttpGet]
        public IEnumerable<TestMasterDTO> Get(string Compid, string TCatName, string TCatID, string IpdID,string OpdID,string Topyn, string Insltude, string Inuserid)
        {
            TestMasterDTO model=new TestMasterDTO();

            Int64 compid = Convert.ToInt64(Compid);
            Int64 TCategoryID = Convert.ToInt64(TCatID);
            Int64 ipdid = Convert.ToInt64(IpdID);
            Int64 opdid = Convert.ToInt64(OpdID);

            Int64 INSuserid = Convert.ToInt64(Inuserid);

            var ache_kina_data = (from n in db.HMS_TESTMST where n.COMPID == compid && n.TCATNM == TCatName && n.TCATID == TCategoryID && n.IPDID == ipdid && n.OPDID==opdid select n).ToList();
            if (ache_kina_data.Count == 0)
            {
                ...
                if (name_ache_kina.Count == 0)
                {
                    ...
                    if (max_idta_berkorbo == 0)
                    {
                        ...
                        model.IpdId = obj.IPDID;
                        model.OpdId = obj.OPDID;
            ...
            {
                var ff = (from n in db.HMS_TEST where n.COMPID==compid && n.TCATID == TCategoryID && n.IPDID==ipdid && n.OPDID==opdid select n).ToList();
        ...
        public HttpResponseMessage TestChild(TestMasterDTO model)
        {
            ...
            var check_data = (from n in db.HMS_TEST where n.COMPID==model.COMPID && n.TCATID == model.TestCatId && n.IPDID == model.IpdId
                                  && n.OPDID==model.OpdId && n.TESTNM==model.TestName select n).ToList();
            if (check_data.Count == 0)
            {
                var ache_kina_data = (from n in db.HMS_TEST where n.COMPID==model.COMPID && n.TCATID == model.TestCatId && n.IPDID==model.IpdId && 
                                      n.OPDID==model.OpdId select n).ToList();
                ...
                if (ache_kina_data.Count == 0)
                {
                    ...
                    testData.TCATID = model.TestCatId;
                    testData.IPDID = model.IpdId;
                    testData.OPDID = model.OpdId;
                    ...
                    model.IpdId = Convert.ToInt64(testData.IPDID);
                    model.OpdId = Convert.ToInt64(testData.OPDID);
                ...
                {
                    Int64 max_idta_berkorbo = Convert.ToInt64((from n in db.HMS_TEST where n.COMPID==model.COMPID && n.TCATID == model.TestCatId && n.IPDID==model.IpdId &&
                                                               n.OPDID==model.OpdId select n.TESTID).Max());
                    Test testData = new Test();
        ...
        public HttpResponseMessage SaveData(TestMasterDTO model)
        {
            ...
            var duplicate_data_test =
                    (from n in db.HMS_TEST where n.COMPID==model.COMPID && n.TCATID == model.TestCatId && n.IPDID == model.IpdId 
                        && n.OPDID==model.OpdId && n.TESTNM==model.TestName select n).ToList();
            Int64 id = 0;
            if (duplicate_data_test.Count > 0)
            {
                ...
                if (id == model.ID)
                {
                    ...
                    foreach (var item in data_find)
                    {
                        ...
                        item.IPDID = model.IpdId;
                        item.OPDID = model.OpdId;
        ...
        public IEnumerable<TestMasterDTO> GetHead(string Compid)
        {
            ...
            //                  join t2 in db.HMS_HEADIO on t1.COMPID equals t2.COMPID into xx
            ...
            //                      ipdid = t1.IPDID,
            //                      ipdname = (subpet.HEADID == t1.IPDID ?subpet.HEADNM :"" ),
                                
            //                      opdid=t1.OPDID,
            //                      opdname = (subpet.HEADID == t1.OPDID ? subpet.HEADNM : ""),
            //                      topyn = t1.TOPYN


            //                  });
          
            var testmasterdata = (from n in db.HMS_TESTMST where n.COMPID == compid select n).ToList();
            foreach (var testMaster in testmasterdata)
            {
                var search_Ipd =
                    (from n in db.HMS_HEADIO where n.COMPID == compid && n.HEADID == testMaster.IPDID select n).ToList();
                string ipdname = "", opdname = "";
                foreach (var headIo in search_Ipd)
                {
                    ipdname = headIo.HEADNM;
                }
                var search_opd =
                   (from n in db.HMS_HEADIO where n.COMPID == compid && n.HEADID == testMaster.OPDID select n).ToList();
                foreach (var headIo in search_opd)
                {
                    opdname = headIo.HEADNM;
                }

                yield return new TestMasterDTO
                {
                    ID = testMaster.ID,
                    COMPID = testMaster.COMPID,
                    TestCatId = testMaster.TCATID,
                    TestCatName =testMaster.TCATNM,
                    IpdId = testMaster.IPDID,
                    IpdName = ipdname,
                    OpdId = testMaster.OPDID,
                    OpdName =opdname,
                    TopYesNo = testMaster.TOPYN
            ...
            //        IpdId = Convert.ToInt64(item.ipdid),
            //        IpdName = item.ipdname,
            //        OpdId = item.opdid,
        ...
        public HttpResponseMessage SaveData2(TestMasterDTO model)
        {
            ...
            var duplicate_data_test =
                    (from n in db.HMS_TESTMST where n.COMPID == model.COMPID && n.TCATID == model.TestCatId && n.IPDID == model.IpdId
                     && n.OPDID==model.OpdId select n).ToList();
            Int64 id = 0;
            if (duplicate_data_test.Count > 0)
            {
                ...
                if (id == model.ID)
                {
                    ...
                    foreach (var testitem in test_data)
                    {
                        ...
                        testitem.IPDID = model.IpdId;
                        testitem.OPDID = model.OpdId;
