...
namespace AslHMS.Controllers.Api
{
    public class ApiMgtController : ApiController
    {
        ...
        //................................................for update....................
        [System.Web.Http.Route("api/ApiMgt/GetData2")]
        [System.Web.Http.HttpGet]
        public IEnumerable<ManagementDTO> Get2(string Compid)
        {

            Int64 compid = Convert.ToInt64(Compid);


            var ache_kina_data = (from n in db.HMS_MGT where n.COMPID == compid select n).ToList();
            if (ache_kina_data.Count == 0)
            {
            ...
            else
            {
                ...
                foreach (var item in ache_kina_data)
                {
                    yield return new ManagementDTO
                    {
                        ID = item.ID,
                        COMPID = compid,
                       ManagementId = item.MGTID,
                       ManagementName = item.MGTNM,
        ...
        [Route("api/ApiMgt/SaveData2")]
        [HttpPost]
        public HttpResponseMessage SaveData2(ManagementDTO model)
        {
            if (!ModelState.IsValid)
            {
                return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ModelState);
            ...
            var duplicate_data_test =
                    (from n in db.HMS_MGT where n.COMPID == model.COMPID && n.MGTNM == model.ManagementName select n).ToList();
            ...
            if (duplicate_data_test.Count > 0)
            {
                ...
                if (id == model.ID)
                {
                    ...
                    var data_find = (from n in db.HMS_MGT where n.ID == model.ID select n).ToList();
                    foreach (var item in data_find)
                    {
                        item.ID = model.ID;
                        item.COMPID = model.COMPID;
                        item.MGTID = item.MGTID;
                        
                        item.MGTNM = model.ManagementName;
                        item.DESIG = model.Designation;
            ...
            {
                var find_row = (from n in db.HMS_MGT where n.COMPID == model.COMPID && n.ID == model.ID select n).ToList();
