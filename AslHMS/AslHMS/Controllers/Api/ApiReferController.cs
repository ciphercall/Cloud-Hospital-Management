...
namespace AslHMS.Controllers.Api
{
    public class ApiReferController : ApiController
    {
        ...
        public IEnumerable<ReferDTO> Get(string Compid, string ReferGroupId, string ReferId, string ReferName, string Title, string Address, string MobileNo1, string MobileNo2, string EmailId)
        {
            ...
            if (ache_kina_data.Count == 0)
            {
                ...
                if (max_idta_berkorbo == 0)
                {
                    ...
                    {
                        COMPID = compid,
                        REFERGID = Convert.ToInt64(Convert.ToString(compid) + "2010101"),
                        REFERID = Convert.ToInt64(Convert.ToString(compid) + "201010100001"),
                        REFERNM = ReferName,
                        TITLE = Title,
                        ADDRESS = Address,
                        MOBNO1 = MobileNo1,
                    ...
                    yield return new ReferDTO
                    {
                        ...
                        COMPID = compid,
                        ReferName = ReferName,
                        ReferGroupId = Convert.ToInt64(Convert.ToString(compid) + "2010101"),
                        ReferId = Convert.ToInt64(Convert.ToString(compid) + "201010100001"),
                        Title = Title,
                        Address = Address,
                        MobileNo1 = MobileNo1,
        ...
        public HttpResponseMessage DeleteData(ReferDTO model)
        {
            ...
            string query = string.Format("DELETE FROM HMS_RFPCNT WHERE ID='{0}'", model.ID);
