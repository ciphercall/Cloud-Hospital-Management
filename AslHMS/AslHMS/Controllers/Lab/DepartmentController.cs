    ...
    public class DepartmentController : AppController
    {
        ...
        public ActionResult Create(DepartmentDTO model)
        {
            ...
            if (model.DepartmentName != null)
            {
                ...
                {
                    Int64 maxdatafind =0 ;
                    if (model.DepartmentType == "LABORDER")
                    {
                        maxdatafind = Convert.ToInt64((from n in db.HMS_DEPT where n.COMPID == model.COMPID && n.DEPTTP == "LABORDER" select n.DEPTID).Max());
                    }
                    else
                    {
                        maxdatafind = Convert.ToInt64((from n in db.HMS_DEPT where n.COMPID == model.COMPID && n.DEPTTP == "PHARMACY" select n.DEPTID).Max());
                    }
                    if (maxdatafind == 0)
                    {
                        ...
                        if (model.DepartmentType == "LABORDER")
                        {
                            objDept.DEPTID = Convert.ToInt64(Convert.ToString(model.COMPID) + "101");
                        }
                        else
                        {
                            objDept.DEPTID = Convert.ToInt64(Convert.ToString(model.COMPID) + "201");
                        }
                      
                        objDept.DEPTNM = model.DepartmentName;
                        objDept.DEPTTP = model.DepartmentType;
        ...
        public JsonResult DynamicautocompleteDept(string changedText)
        {
            using (var db = new AslHMSApiContext())
            {
                ...
                var result = new { DepartmentName = name, DeptID = DeptId, REMARKS = Remarks, DepartmentType = DeptType };
